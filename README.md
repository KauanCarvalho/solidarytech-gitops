...
| 1.5 | Evidência de redução de MTTR | ✅ | `self-healing.yml` executado de ponta a ponta ([run 35164304583](https://github.com/KauanCarvalho/solidarytech-app/actions/runs/35164304583)) — rollout restart do `donation-service` de disparo a `successfully rolled out` em **~22s**; drill de DR completo (namespace inteiro recriado via ArgoCD selfHeal) em ~70-90s (`docs/PCN.md`) | Dois pontos de dado reais medidos, não estimados: resposta a um único deployment degradado (self-healing) vs. recuperação de um namespace inteiro (DR) |
...
| 2.3 | Forecast de custo mensal + recomendação | ✅ | `docs/finops-forecast.md` — ~$230-240/mês, recomendação de node groups separados por perfil de risco (On-Demand dedicado ao `donation-service`, Spot para os demais) | Otimização de custo sem contradizer o requisito de "doações não podem parar" do próprio edital |
| 3.1 | AIOps ativo (Watchdog) | ✅ | `docs/evidencias/itsm-datadog-watchdog.png` — Watchdog detectou automaticamente (sem threshold manual) 7 alertas de aumento de error rate em `donation-service`/`ngo-service`, classificados como "Critical Failure" | Anomalia real gerada durante troubleshooting, pega pela IA do APM sem configuração de alerta específico — é a "gestão preditiva" que o edital pede, não um monitor estático |
